...
.  Use the `AddressBookParser` class to parse the user command.
...
The following sequence diagram (Fig. 12) expands on the Parse Sequence block in the previous diagram.
...
As the user continues to issue more commands, they are added into the `undoStack`. For example, the user may execute `add n/BTC` to add a new coin, resulting in the below picture (Fig. 13):
...
The diagram below (Fig. 23) summarises these interactions:
